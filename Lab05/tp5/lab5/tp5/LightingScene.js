...
LightingScene.prototype.init = function(application)
{
	...
	this.clock = new ClockObject(this);

	this.paperPlanes = new PlanesObject(this, 5);
	...
	this.textureClock = new CGFappearance(this);
	this.textureClock.setTextureWrap('CLAMP_TO_EDGE', 'CLAMP_TO_EDGE');
	this.textureClock.loadTexture("../resources/images/clock.png");

	//Animations - update scene interval
...
{
	var currentDateTime = new Date(currTime); 

	//var seconds = (currTime / 1000).toFixed(1);
	//var minutes = (currTime / (1000 * 60)).toFixed(1);
    //var hours = (currTime / (1000 * 60 * 60)).toFixed(1);
    //var days = (currTime / (1000 * 60 * 60 * 24)).toFixed(1);

	//console.log(currTime + ' -> ' + days+' - '+hours+':'+minutes+':'+seconds);
	//console.log(currentdate);

	//this.clock.setTime(12, 30, 15, 750);	//test
	this.clock.setTime(currentDateTime.getHours(), currentDateTime.getMinutes(), currentDateTime.getSeconds(), currentDateTime.getMilliseconds());

	this.paperPlanes.launchPlanes(0.5);
}
...
LightingScene.prototype.display = function()
{
	...
	//Paper Planes
	this.pushMatrix();
		this.translate(15, 6, 7);
		this.rotate(-90 * degToRad, 0, 1, 0);
		this.materialDefault.apply();
		this.paperPlanes.display();
	this.popMatrix();
